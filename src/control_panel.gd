...
## 怪物菜单
@onready var monster_spawn_option_button: OptionButton = $"选项卡切换/怪物菜单/H/V2/H2/生成新怪物下拉菜单"
## 其他菜单
...
func _process(_delta: float) -> void:
	## 和平模式且场上有怪物下，按钮不能按
	self.yoburin_peaceful_mode_checkbutton.disabled = true if self.peaceful_mode and get_tree().get_nodes_in_group(GROUP_MONSTERS) else false
	
	
## 生成怪物生成下拉菜单
func _generate_monster_spawn_options() -> void:
	for type in GameManager.MonsterType.keys():
		self.monster_spawn_option_button.add_item(GameManager.MonsterTypeMapping[type])
